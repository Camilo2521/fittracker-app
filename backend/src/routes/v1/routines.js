...
router.post('/generate', async (req, res) => {
  ...
  // RAG path — sólo si el flag está activo Y el usuario existe en la DB
  if (FLAGS.rag_enabled && user) {
    ...
    // Si el servicio RAG falla, caemos al generador local
  }

  // Generador local basado en plantillas
  ...
  try {
    const result = await pg.query(
      `SELECT r.*, json_agg(
       ...
       WHERE r.external_id = $1 AND r.is_active = true
       GROUP BY r.id
       LIMIT 1`,
      [req.params.userId]
    );
